...
$null  = $lines.Add("# ---------------------------------------------------------------------------")
$null  = $lines.Add("#  posh runtime configuration — generated by tools\Initialize-PoshConfig.ps1")
...
$null  = $lines.Add("#    .\Edit-PoshSettings.ps1           # browser-based editor (recommended)")
...
# the convention from tools\New-PoshAesKey.ps1. Rotate so only the last 5
# backups stick around — otherwise repeated -Force runs leave a growing
# trail of config.psd1.bak.* in the repo root.
# ---------------------------------------------------------------------------
...
try {
    ...
    Write-Host '         Inspect the file and report this as a bug in tools\Initialize-PoshConfig.ps1.'
    exit 1
}
