...
`config.psd1` is generated by `tools\Initialize-Config.ps1` and has every key. The editor renders the subset the schema documents (Server-Ports, Auth, Rate-Limit, Logging, Komfort-Features). Removing a key by hand makes it disappear from the editor; restore with `Initialize-Config.ps1 -Force`.

### Setup-Helfer (separate tab)
