...
Prerequisite: `config.psd1` exists (run [tools/Initialize-PoshConfig.ps1](../tools/Initialize-PoshConfig.ps1) once if not).
...
- You click **Quit** in the UI (preferred)
- You click **Save** and the save succeeds (5 s grace, then auto-quit)
...
`globalvars.ps1` and `config.psd1` — together.

### globalvars.ps1 — dynamic
...
`config.psd1` is generated by `tools\Initialize-PoshConfig.ps1` and has every key. The editor renders the subset the schema documents (server ports, auth, rate-limit, logging, quality-of-life features). Removing a key by hand makes it disappear from the editor; restore with `Initialize-PoshConfig.ps1 -Force`.

### Setup helpers (separate tab)

- Generate AES key (`$key` in `globalvars.ps1`)
- Store encrypted secret (`encrypted_pw\encryptedString_<label>.txt`)
- Set `POSH_API_KEY` machine env-var (admin-only)

### Out of scope on purpose (open the file by hand)

- The `$key` byte array (only the generator button writes it; `Remove` refuses)
- `BackgroundJobs` (nested hashtable array in `config.psd1`)
...
1. Click **Save** when one or more fields are dirty.
...
Run [tools/Initialize-PoshConfig.ps1](../tools/Initialize-PoshConfig.ps1) once. The server itself also hard-fails at startup without `config.psd1` — that's by design.

**The "Set POSH_API_KEY" button is disabled.**
